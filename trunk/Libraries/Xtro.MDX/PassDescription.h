...
namespace Direct3D10
{
	...
	{
	internal:
		PassDescription(D3D10_PASS_DESC* Native)
		{
			...
			BlendFactor = Float4(Native->BlendFactor);
		}
		
		D3D10_PASS_DESC ToNative()
		{
			...
			BlendFactor.ToNative(Result.BlendFactor);
	...
	public:
		...
		PassDescription(String^ Name, unsigned int Annotations, array<Byte>^ IAInputSignature, unsigned int StencilRef, unsigned int SampleMask, Float4 BlendFactor)
		{		
			this->Name = Name;
			this->Annotations = Annotations;
			this->IAInputSignature = IAInputSignature;
