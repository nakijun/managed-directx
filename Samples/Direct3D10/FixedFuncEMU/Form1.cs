...
namespace FixedFuncEMU
{
    ...
    public partial class Form1 : Form
    {
        ...
        int OnDeviceCreated(Device Device, ref SurfaceDescription BackBufferSurfaceDesc, object UserContext)
        {
            ...
            // Load meshes
            Result = ObjectMesh.Create(Device, "FixedFuncEMU\\TankScene.sdkmesh", true);
            if (Result < 0) return Result;
            Result = SkyMesh.Create(Device, "FixedFuncEMU\\desertsky.sdkmesh", true);
            if (Result < 0) return Result;
            ...
            Result = UtilitiesFunctions.FindSDK_MediaFileCch(out DestinationPath, "FixedFuncEMU\\smokevol1.dds");
            if (Result < 0) return Result;
            Result = D3DX10Functions.CreateShaderResourceViewFromFile(Device, DestinationPath, out ParticleTextureRV);
            if (Result < 0) return Result;

            Result = UtilitiesFunctions.FindSDK_MediaFileCch(out DestinationPath, "FixedFuncEMU\\colorgradient.dds");
            if (Result < 0) return Result;
