...
namespace Tutorial13
{
    public partial class Form1 : Form
    {
        ...
        EffectScalarVariable ExplodeVariable = null;
        ...
        void Init()
        {
            ...
            Y += 24;
            var Text = string.Format("Explode Amount: {0:0.00}", Explode);
            Static CreatedStatic;
            SampleUI.AddStatic((int)ControlID.ExplodeStatic, Text, 35, Y += 24, 125, 22, false, out CreatedStatic);
            Slider CreatedSlider;
            SampleUI.AddSlider((int)ControlID.ExplodeScale, 50, Y += 24, 100, 22, 0, 3000, (int)(Explode * 100.0f), false, out CreatedSlider);
        ...
        int OnDeviceCreated(Device Device, ref SurfaceDescription BackBufferSurfaceDescription, object UserContext)
        {
            ...
            Result = UtilitiesFunctions.FindSDK_MediaFileCch(out DestinationPath, "Tutorial13.fx");
            if (Result < 0) return Result;
            ...
            ExplodeVariable = Effect.GetVariableByName("Explode").AsScalar();

            // Set Waviness
            ExplodeVariable.SetFloat(Explode);
        ...
        void OnGUI_Event(Event Event, int ControlID, Control Control, object UserContext)
        {
            ...
            case (int)Form1.ControlID.ExplodeScale:
                {
                    Explode = SampleUI.GetSlider((int)Form1.ControlID.ExplodeScale).GetValue() * 0.01f;
                    var S = string.Format("Explode Amount: {0:0.00}", Explode);
                    SampleUI.GetStatic((int)Form1.ControlID.ExplodeStatic).Text = S;
                    ExplodeVariable.SetFloat(Explode);
                    break;
                }
            }
